...
use plushie_core::extensions::ExtensionDispatcher;
...
pub struct App {
    pub core: plushie_core::engine::Core,
    pub theme: Theme,
    ...
    pub image_registry: plushie_core::image_registry::ImageRegistry,
...
impl App {
    pub fn new(dispatcher: ExtensionDispatcher, effect_handler: Box<dyn EffectHandler>) -> Self {
        Self {
            core: plushie_core::engine::Core::new(),
            theme: DEFAULT_THEME,
            pending_tasks: Vec::new(),
            windows: window_map::WindowMap::new(),
            image_registry: plushie_core::image_registry::ImageRegistry::new(),
    ...
    pub fn title_for_window(&self, window_id: window::Id) -> String {
        if let Some(plushie_id) = self.windows.get_plushie(&window_id)
            && let Some(node) = self.core.tree.find_window(plushie_id)
    ...
    pub fn theme_ref_for_window(&self, window_id: window::Id) -> &Theme {
        if let Some(plushie_id) = self.windows.get_plushie(&window_id)
            && let Some(cached) = self.windows.cached_theme(plushie_id)
        {
            return cached;
        }
