...
pub struct App {
    ...
    pub(crate) last_slide_values: HashMap<String, f64>,
    /// Extension dispatcher for custom widget types.
    pub(crate) dispatcher: ExtensionDispatcher,
    /// Epoch for animation_frame timestamp calculation.
    pub(crate) animation_epoch: Option<iced::time::Instant>,
    ...
    pub(crate) effect_handler: Box<dyn EffectHandler>,
}
...
impl App {
    ...
    pub(crate) fn theme_ref_for_window(&self, window_id: window::Id) -> &Theme {
        if let Some(toddy_id) = self.windows.get_toddy(&window_id)
    ...
    pub(crate) fn emit_subscription(
        &self,
        key: &str,
        captured: bool,
    ...
    pub(crate) fn lookup_widget_event_rate(&self, widget_id: &str) -> Option<u32> {
    ...
    pub(crate) fn coalesce_subscription(
        &mut self,
        key: &str,
        captured: bool,
        event_fn: impl FnOnce(String) -> OutgoingEvent,
